...
from standard input (cin)
    ...
    create an array of pointers, maximum 100, to Pet objects for each pet that you read in

once you've read in each pet and created a pet object and stored address in your array, read in a list of integers, one per line, and for each integer call the corresponding pet's speak, eat and sit methods
...
public:
    ...
    virtual string name() const { return petName; }
...
int main(int argc, char** argv)
{
    ...
    while (petNum > 0 && petNum <= numOfPets)
    {
        //inside of loop call the speak, eat, and sit methods of the pet and output what each one returns
        cout << p[petNum - 1]->speak() << endl;
        cout << p[petNum - 1]->eat() << endl;
        cout << p[petNum - 1]->sit() << endl;
